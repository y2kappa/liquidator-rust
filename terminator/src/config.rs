...
pub fn get_client_for_action(args: &Args) -> Result<KlendClient> {
    let (payer, placeholder) = get_keypair_for_action(&args.action, &args.keypair)?;
...
pub fn get_keypair_for_action(
    ...
    validate_keypair_for_action(action, &keypair)?;
    Ok((keypair, pubkey))
}
...
fn validate_keypair_for_action(action: &Actions, keypair: &Option<Keypair>) -> Result<()> {
    match action {
        _ => {
            if keypair.is_none() {
