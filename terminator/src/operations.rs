...
use kamino_lending::{
    utils::{seeds::BASE_SEED_REFERRER_TOKEN_STATE, Fraction},
    LendingMarket, Obligation, ReferrerTokenState, Reserve,
};
...
pub fn refresh_reserve<'a>(
    ...
    if kamino_lending::lending_market::lending_operations::is_price_refresh_needed(
        ...
        let px = kamino_lending::utils::prices::get_price(
            ...
            pyth_oracle.as_ref(),
            switchboard_price_oracle.as_ref(),
            switchboard_twap_oracle.as_ref(),
            scope_prices.as_ref(),
            clock.unix_timestamp,
        )?;
...
pub struct ObligationReserves {
    pub obligation_reserves: Vec<StateWithKey<Reserve>>,
...
pub fn obligation_reserves(
    ...
    let borrow_reserves_cloned = borrow_reserves.clone();

    let obligation_reserves = deposit_reserves
        .into_iter()
        .chain(borrow_reserves)
        .collect::<Vec<_>>();

    Ok(ObligationReserves {
        obligation_reserves,
        borrow_reserves: borrow_reserves_cloned,
    })
}
...
pub fn split_obligations(obligations: &[(Pubkey, Obligation)]) -> SplitObligations {
    let (zero_debt, risky) = obligations.iter().partition(|(_, obligation)| {
        Fraction::from_bits(obligation.borrow_factor_adjusted_debt_value_sf).is_zero()
    });
