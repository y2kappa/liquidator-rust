...
use crate::{
    ...
    utils::{get_all_reserve_mints, model::StateWithKey},
};
...
async fn rebalance(klend_client: &Arc<KlendClient>) -> Result<()> {
    ...
    let amount = 100.0;
    let pxs =
        utils::px::fetch_jup_prices(&liquidity_mints, &rebalance_config.usdc_mint, amount).await?;
    info!("Loading holdings..");
    let mut holdings = klend_client
        .liquidator
        .fetch_holdings(&klend_client.client, &all_reserves, &pxs)
        .await?;
...
pub mod swap {
    ...
    pub async fn swap_action(
        ...
        let pxs = utils::px::fetch_jup_prices(&l_mints, &rebalance_config.usdc_mint, amount as f32)
            .await?;
        let holdings = klend_client
            .liquidator
            .fetch_holdings(&klend_client.client, &reserves, &pxs)
            .await?;
    ...
    pub async fn swap(
        ...
        let tx = VersionedTransaction::try_new(
            VersionedMessage::V0(v0::Message::try_compile(
                ...
                klend_client.client.get_latest_blockhash().await?,
            )?),
            &[klend_client.liquidator.wallet.as_ref()],
        )?;
...
async fn liquidate(klend_client: &KlendClient, obligation: &Pubkey) -> Result<()> {
    ...
    let clock = utils::sysvars::clock(&klend_client).await;
    ...
    let pxs = utils::px::fetch_jup_prices(&[debt_mint], &rebalance_config.usdc_mint, 100.0).await?;
    let holdings = klend_client
        .liquidator
        .fetch_holdings(&klend_client.client, &reserves, &pxs)
        .await?;
...
async fn crank(klend_client: &KlendClient, obligation_filter: Option<Pubkey>) -> Result<()> {
    ...
    loop {
        for market in &markets {
            ...
            let clock = utils::sysvars::get_clock(&klend_client).await.unwrap();
