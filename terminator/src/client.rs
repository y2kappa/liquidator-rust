...
use crate::{
    ...
    utils::{
        ...
        model::StateWithKey,
        px::{fetch_jup_prices, Prices},
    },
};

use self::rpc::get_all_zero_copy_accounts;
...
impl KlendClient {
    ...
    pub async fn fetch_referrer_token_states(&self) -> Result<HashMap<Pubkey, ReferrerTokenState>> {
        let states = get_all_zero_copy_accounts::<ReferrerTokenState>(self).await?;
    ...
    async fn load_liquidator_lookup_table(&mut self) {
        ...
        if current_content.is_empty() {
            ...
            let lookup_table_data = self.client.get_account(&lut_key).await.unwrap();
    ...
    async fn create_init_reserve_lookup_table(
        ...
        let txn = VersionedTransaction::try_new(
            VersionedMessage::V0(v0::Message::try_compile(
                &self.liquidator.wallet.pubkey(),
                &[create_lookup_table],
                &[],
                self.client.get_latest_blockhash().await?,
            )?),
            &[self.liquidator.wallet.as_ref()],
        )?;
    ...
    async fn extend_lut_with_keys(
        ...
        for selected_keys in keys.chunks(20) {
            ...
            let extend_ix = instruction::extend_lookup_table(
                table_pk,
                self.payer.as_ref().unwrap().pubkey(),
                Some(self.payer.as_ref().unwrap().pubkey()),
    ...
    pub async fn fetch_all_prices(
        ...
        let amount = 100.0;
        fetch_jup_prices(&mints, usd_mint, amount).await
    }
    ...
    pub async fn liquidate_obligation_and_redeem_reserve_collateral(
        ...
        let txn_b64 = bs58::encode(txn.message.serialize()).into_string();
        println!(
    ...
    pub async fn wrap_obligation_instruction_with_farms(
        ...
        for reserve in reserve_accts {
            ...
            let (obligation_farm_debt, obligation_farm_coll) =
                obligation_farms(&self, farm_debt, farm_collateral, obligation_address).await;
        ...
        for reserve_acc in unique_reserves {
            ...
            let reserve: Reserve = self.get_anchor_account(&reserve_acc).await.unwrap();
        ...
        for (reserve_acc, farm_mode) in reserve_accts.iter().zip(farm_modes.iter()) {
            let reserve: Reserve = self.get_anchor_account(&reserve_acc.key).await.unwrap();
        ...
        let referrer_token_states: Vec<Option<Pubkey>> = if obligation.has_referrer() {
            ...
            for borrow_reserve in borrow_reserves.iter() {
                match borrow_reserve {
                    Some(borrow_reserve) => {
                        let reserve_account: Reserve =
                            self.get_anchor_account(borrow_reserve).await.unwrap();
...
pub mod rpc {
    ...
    pub async fn get_account(klend: &KlendClient, pubkey: &Pubkey) -> Result<Account> {
        Ok(<RpcClient>::get_account_with_commitment(
            &klend.client,
            pubkey,
    ...
    pub async fn get_all_zero_copy_accounts<Acc>(klend: &KlendClient) -> Result<Vec<(Pubkey, Acc)>>
    where
        Acc: AnyBitPattern + Owner + Discriminator,
    {
        let size = u64::try_from(std::mem::size_of::<Acc>() + 8).unwrap();
        let accounts = get_program_accounts_with_size_and_discriminator(
            klend,
            &Acc::owner(),
            size,
            &Acc::DISCRIMINATOR,
    ...
    async fn get_program_accounts_with_size_and_discriminator(
        ...
        <RpcClient>::get_program_accounts_with_config(&klend.client, program_id, config)
            .await
            .map_err(Into::into)
    }
}
