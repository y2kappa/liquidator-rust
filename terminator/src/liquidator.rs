...
use crate::{
    accounts::find_account, client::KlendClient, consts::WRAPPED_SOL_MINT, utils::px::Prices,
};
...
async fn get_or_create_ata(
    ...
    if !matches!(find_account(&client.client, ata).await, Ok(None)) {
        ...
        let tx = VersionedTransaction::try_new(
            VersionedMessage::V0(v0::Message::try_compile(
                &client.liquidator.wallet.pubkey(),
                &[ix],
                &[],
                client.client.get_latest_blockhash().await?,
            )?),
            &[client.liquidator.wallet.as_ref()],
        )?;
